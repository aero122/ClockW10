...
//При завершении формы записываем параметры
...
begin
  ...
  Label1.Font.Name:=ini.ReadString('FontStyle','Name','Arial');//Шрифт
  if ini.ReadBool('Position','AlwaysOnTop',True) then//Поверх всех окон
    begin
      Form1.FormStyle:=fsStayOnTop;
      N2.Caption:=WideChar($2713)+' Поверх всех окон';
    end;
  ini.Free;
...
//Каждые три секунды меняем время
...
procedure TForm1.TrayIcon1MouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
//При наведении на иконку в трее генерируем сообщение
var
  rnd:byte;
  hint:String;
begin
  ...
  rnd:=Random(10);
  case rnd of
  0:hint:='Я всё запомнил!';
  1:hint:='Зачем Вы это сделали?';
  2:hint:='Может хватит?';
  3:hint:='Онотоле негодуе';
  4:hint:='Ответ на все вопросы - 48';
  5:hint:='Счастливые часов не замечают';
  6:hint:='Мне надоело менять циферки';
  7:hint:='Это просто часы';
  8:hint:='Самые обычные часы и ничего больше';
  9:hint:='Самое время отдохнуть';
  end;
  TrayIcon1.Hint:=hint;
end;
